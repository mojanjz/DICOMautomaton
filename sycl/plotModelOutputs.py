...
with open('data/input/c_noise.txt') as f: # grab our data generated with presence of noise
    lines = f.readlines()
    cNT = [float(line.split()[0]) for line in lines]
    cNVal = [float(line.split()[2]) for line in lines]

with open('data/input/sanitized_c.txt') as f: # grab our data generated with presence of noise
    lines = f.readlines()
...
ax1.set_title("Intensity of Contrast Enhancement in Current Voxel")    
ax1.set_xlabel('Time (s)')
ax1.set_ylabel('Intensity')
# ax1.plot(cT,cVal, c='r', label='Our output')
ax1.plot(cET,cEVal, c='b', label='Model Output')
ax1.plot(cNT,cNVal, c='g', label='Noisy Input')
ax1.plot(cST,cSVal, c='r', label='Sanitized Input')
